...
import com.easemob.chatuidemo.ChatConstant;
...
public class MainActivity extends BaseActivity implements EMEventListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(savedInstanceState != null && savedInstanceState.getBoolean(ChatConstant.ACCOUNT_REMOVED, false)){
...
//		MobclickAgent.setDebugMode( true );
		...
		if (getIntent().getBooleanExtra("conflict", false) && !isConflictDialogShow){
			showConflictDialog();
		}else if(getIntent().getBooleanExtra(ChatConstant.ACCOUNT_REMOVED, false) && !isAccountRemovedDialogShow){
   ...
   @Override
	...
	public int getUnreadAddressCountTotal() {
		...
		if (DemoApplication.getInstance().getContactList().get(ChatConstant.NEW_FRIENDS_USERNAME) != null)
			unreadAddressCountTotal = DemoApplication.getInstance().getContactList().get(ChatConstant.NEW_FRIENDS_USERNAME).getUnreadMsgCount();
	...
	private void saveInviteMsg(InviteMessage msg) {
		...
		// 未读数加1
		User user = DemoApplication.getInstance().getContactList().get(ChatConstant.NEW_FRIENDS_USERNAME);
